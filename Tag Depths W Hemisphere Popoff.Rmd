...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234811 with 234811) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234811 <-read.csv(file="C:/R_git_stuff/Mobula-tag-depths/234811-Series.csv", header=TRUE, sep=",")

#head(Depths_234811)
#glimpse(Depths_234811)
#dim(Depths_234811) 
```
...
```{r}
Depths_234811$datetime = dmy_hms(paste(Depths_234811$Day,Depths_234811$Time))
```
...
Popoff_loc_234811 <- Popoff_loc %>% 
  filter(Ptt == 234811) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Ptt = as.integer(234811))

Popoff_loc_234811$Popoff_tz <- tz_lookup_coords(Popoff_loc_234811$Popoff_lat, Popoff_loc_234811$Popoff_long, method = "accurate")

Depths_234811 <- left_join(Depths_234811,Popoff_loc_234811, by = join_by (Ptt))
```
...
```{r}
Depths_234811$Popoff_datetime = Depths_234811$datetime %>%
...
z <- Depths_234811$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234811$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234811$Popoff_datetime[na_rows]
```
*****************************************************************************************************
#Line Plots
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234811, "Etc/GMT+10", "Tag ID 234811", x_label = "Date/time (HST))", ylims = c(400, 0))

ggsave("234811_lineplot.png")
```
...
```{r}
Depths_234811_filtered <-Depths_234811 %>% dplyr::filter(datetime > ymd_hms("2024-05-31 00:00:00", tz="UTC") & datetime < ymd_hms("2024-06-04 09:20:00", tz="UTC"))#Need to filter out with actual date/time in data frame and use last time before tag floating.
```

### Change time zone and label for time offset before running code.
```{r}
brett_depth_plot_good_label_yaxis(Depths_234811_filtered, "Etc/GMT+10", "Tag ID 234811", x_label = "Date/time (HST)", ylims = c(400, 0))

ggsave("234811_lineplot.jpg")
```
**************************************************************************
#BOXPLOTS
...
brett_box_plot(Depths_234811_filtered, "Etc/GMT+10", "Tag ID 234811", x_label = "Hours (HST)", ylims = c(400, 0))
ggsave("234811_boxplot_filtered.png")
...
##Determine if there is any connection between deep dives and high lunar illumination. Can look at the daily max dives from the daily data (e.g. 234811-DailyData.csv) that is reported for the entire tag deployment.  
```{r}
library(lunar)
