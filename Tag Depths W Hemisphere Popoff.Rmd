...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234814 with 234814) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234814 <-read.csv(file="C:/R_git_stuff/Mobula-tag-depths/234814-Series.csv", header=TRUE, sep=",")

#head(Depths_234814)
#glimpse(Depths_234814)
#dim(Depths_234814) 
```
...
```{r}
Depths_234814$datetime = dmy_hms(paste(Depths_234814$Day,Depths_234814$Time))
```
...
Popoff_loc_234814 <- Popoff_loc %>% 
  filter(Ptt == 234814) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Ptt = as.integer(234814))

Popoff_loc_234814$Popoff_tz <- tz_lookup_coords(Popoff_loc_234814$Popoff_lat, Popoff_loc_234814$Popoff_long, method = "accurate")

Depths_234814 <- left_join(Depths_234814,Popoff_loc_234814, by = join_by (Ptt))
```
...
```{r}
Depths_234814$Popoff_datetime = Depths_234814$datetime %>%
...
z <- Depths_234814$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234814$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234814$Popoff_datetime[na_rows]
```
*****************************************************************************************************
#Line Plots
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234814, "Etc/GMT+11", "Tag ID 234814", x_label = "Date/time (HST - 1) hr)", ylims = c(400, 0))

ggsave("234814_lineplot.png")
```
...
```{r}
Depths_234814_filtered <-Depths_234814 %>% dplyr::filter(datetime > ymd_hms("2024-05-31 00:00:00", tz="UTC") & datetime < ymd_hms("2024-06-04 09:20:00", tz="UTC"))#Need to filter out with actual date/time in data frame and use last time before tag floating.

Depths_234814_filtered = Depths_234814 %>% dplyr::filter(datetime > ymd_hms("2023-07-12 02:30:00", tz="UTC") & datetime < ymd_hms("2023-07-14 11:00:00", tz="UTC"))

```

### Change time zone and label for time offset before running code.
```{r}
brett_depth_plot_good_label_yaxis(Depths_234814_filtered, "Etc/GMT+11", "Tag ID 234814", x_label = "Date/time (HST - 1 hrs)", ylims = c(400, 0))

ggsave("234814_lineplot.jpg")
```
**************************************************************************
#BOXPLOTS
...
```{r}
brett_box_plot(Depths_234814, "Etc/GMT+11", "Tag ID 234814",
               x_label = "Hour (HST - 1 hr)", ylims = c(400, 0))
ggsave("234814_boxplot.png")

#run below code if need to filter out bad data.
#brett_box_plot(Depths_234814_filtered, "Etc/GMT+10", "Tag ID 234814", x_label = "Hours (HST)", ylims = c(400, 0))
#ggsave("234814_boxplot_filtered.png")
...
##Determine if there is any connection between deep dives and high lunar illumination. Can look at the daily max dives from the daily data (e.g. 234814-DailyData.csv) that is reported for the entire tag deployment.  
```{r}
library(lunar)
