...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234815 with 234815) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234815 <-read.csv("234815-Series.csv", header=TRUE, sep=",")

#head(Depths_234815)
#glimpse(Depths_234815)
#dim(Depths_234815) 
```
...
```{r}
Depths_234815$datetime = dmy_hms(paste(Depths_234815$Day,Depths_234815$Time))
```
...
Popoff_loc_234815 <- Popoff_loc %>% 
  filter(Tag_PTT == 234815) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Tag_PTT = as.integer(234815)) %>%
  rename(Ptt = Tag_PTT)

Popoff_loc_234815$Popoff_tz <- tz_lookup_coords(Popoff_loc_234815$Popoff_lat, Popoff_loc_234815$Popoff_long, method = "accurate")

Depths_234815 <- left_join(Depths_234815,Popoff_loc_234815, by = join_by (Ptt)) #if run this more than once then will keep joining with new field names. Be careful!
```
...
```{r}
Depths_234815$Popoff_datetime = Depths_234815$datetime %>%
...
z <- Depths_234815$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234815$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234815$Popoff_datetime[na_rows]
```
*****************************************************************************************************
# Line Plots
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234815, "Etc/GMT+11", "Tag ID 234815", ylims = c(400, 0), x_label = "Date/time (HST - 1) hr)")

ggsave("234815_lineplot.png")
```
...
```{r}
Depths_234815_filtered <-Depths_234815 %>% dplyr::filter(datetime > ymd_hms("2024-05-31 00:00:00", tz="UTC") & datetime < ymd_hms("2024-06-04 09:20:00", tz="UTC"))#Need to filter out with actual date/time in data frame and use last time before tag floating.

Depths_234815_filtered = Depths_234815 %>% dplyr::filter(datetime > ymd_hms("2023-07-12 02:30:00", tz="UTC") & datetime < ymd_hms("2023-07-14 11:00:00", tz="UTC"))

```

### Change time zone and label for time offset before running code.
```{r}
brett_depth_plot_good_label_yaxis(Depths_234815_filtered, "Etc/GMT+11", "Tag ID 234815", x_label = "Date/time (HST - 1 hrs)", ylims = c(400, 0))

ggsave("234815_lineplot.jpg")
```
**************************************************************************
#BOXPLOTS
...
```{r}
brett_box_plot(Depths_234815, "Etc/GMT+11", "Tag ID 234815",
               x_label = "Hour (HST - 1 hr)", ylims = c(400, 0))
ggsave("234815_boxplot.png")

#run below code if need to filter out bad data.
#brett_box_plot(Depths_234815_filtered, "Etc/GMT+10", "Tag ID 234815", x_label = "Hours (HST)", ylims = c(400, 0))
#ggsave("234815_boxplot_filtered.png")
...
##Determine if there is any connection between deep dives and high lunar illumination. Can look at the daily max dives from the daily data (e.g. 234815-DailyData.csv) that is reported for the entire tag deployment.  
```{r}
library(lunar)
