...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234816 with 234816) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234816 <-read.csv("234816-Series.csv", header=TRUE, sep=",")

#head(Depths_234816)
#glimpse(Depths_234816)
#dim(Depths_234816) 
```
...
```{r}
Depths_234816$datetime = dmy_hms(paste(Depths_234816$Day,Depths_234816$Time))
```
...
```{r}
Popoff_loc <-read.csv(file="Mobula_locations.csv", header=TRUE, sep=",")

Popoff_loc_234816 <- Popoff_loc %>% 
  filter(Tag_PTT == 234816) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Tag_PTT = as.integer(234816)) %>%
  rename(Ptt = Tag_PTT)

Popoff_loc_234816$Popoff_tz <- tz_lookup_coords(Popoff_loc_234816$Popoff_lat, Popoff_loc_234816$Popoff_long, method = "accurate")

Depths_234816 <- left_join(Depths_234816,Popoff_loc_234816, by = join_by (Ptt)) #if run this more than once then will keep joining with new field names. Be careful!
```
...
```{r}
Depths_234816$Popoff_datetime = Depths_234816$datetime %>%
  lubridate:::ymd_hms(tz="UTC") %>% 
    with_tz(tzone="Etc/GMT+11")

#Issues: You may get an error (Warning: 6 failed to parse) as anything recorded at midnight (00:00:00) will result in NAs. Run below code to add date/times for NAs based on prior or subsequent recorded date/times. I had to run code twice.
```
...
z <- Depths_234816$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234816$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234816$Popoff_datetime[na_rows]
```
*****************************************************************************************************
# Line Plots
...
#### 2) Load .R into Rstudio and hit source button on top right or run below -  source("depth_plot_good_label_updated.R")
...
#### 4) Run the function in the console from the .R file (brett_depth_plot_good_label() and change inputs. Note if run code in console then will go to plots and can export there otherwise will need to run with ggsave to export.
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234816, "Etc/GMT+11", "Tag ID 234816", ylims = c(400, 0), x_label = "Date/time (HST - 1 hr)")

ggsave("234816_lineplot.png")
```

##Filter out any bad data
###Need to examine plot for any data that needs to be filtered out (e.g. when tag is floating or when data missing). For line plots if there is missing data during the middle of the deploy then just note the dates the data was not transmitted. 
...
###Tags to filter: 
#### M. mobular 811 - filter(datetime > ymd_hms("2024-05-31 00:00:00", tz="UTC") & datetime < ymd_hms("2024-06-04 09:20:00", tz="UTC"))#Need to filter out with actual date/time in data frame and use last time before tag floating. 

```{r}
# Depths_234816_filtered <-Depths_234816 %>% dplyr::filter(datetime > ymd_hms("2024-05-31 00:00:00", tz="UTC") & datetime < ymd_hms("2024-06-04 09:20:00", tz="UTC"))#Need to filter out with actual date/time in data frame and use last time before tag floating.
# 
# Depths_234816_filtered = Depths_234816 %>% dplyr::filter(datetime > ymd_hms("2023-07-12 02:30:00", tz="UTC") & datetime < ymd_hms("2023-07-14 11:00:00", tz="UTC"))
```

### Change time zone and label for time offset before running code.
```{r}
# brett_depth_plot_good_label_yaxis(Depths_234816_filtered, "Etc/GMT+11", "Tag ID 234816", x_label = "Date/time (HST - 1 hrs)", ylims = c(400, 0))
# 
# ggsave("234816_lineplot.jpg")
```
**************************************************************************
#BOXPLOTS
...
```{r}
brett_box_plot(Depths_234816, "Etc/GMT+11", "Tag ID 234816",
               x_label = "Hour (HST - 1 hr)", ylims = c(400, 0))
ggsave("234816_boxplot.png")

#run below code if need to filter out bad data.
#brett_box_plot(Depths_234816_filtered, "Etc/GMT+10", "Tag ID 234816", x_label = "Hours (HST)", ylims = c(400, 0))
#ggsave("234816_boxplot_filtered.png")
...
##Determine if there is any connection between deep dives and high lunar illumination. Can look at the daily max dives from the daily data (e.g. 234816-DailyData.csv) that is reported for the entire tag deployment.  
```{r}
library(lunar)
