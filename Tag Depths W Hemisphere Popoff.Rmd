...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234821 with 234821) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234821 <-read.csv(file="C:/R_git_stuff/Mobula-tag-depths/234821-Series.csv", header=TRUE, sep=",")

#head(Depths_234821)
#glimpse(Depths_234821)
#dim(Depths_234821) 
```
...
```{r}
Depths_234821$datetime = dmy_hms(paste(Depths_234821$Day,Depths_234821$Time))
```
...
Popoff_loc_234821 <- Popoff_loc %>% 
  filter(Ptt == 234821) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Ptt = as.integer(234821))

Popoff_loc_234821$Popoff_tz <- tz_lookup_coords(Popoff_loc_234821$Popoff_lat, Popoff_loc_234821$Popoff_long, method = "accurate")

Depths_234821 <- left_join(Depths_234821,Popoff_loc_234821, by = join_by (Ptt))
```
...
```{r}
Depths_234821$Popoff_datetime = Depths_234821$datetime %>%
...
z <- Depths_234821$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234821$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234821$Popoff_datetime[na_rows]
```
*****************************************************************************************************
#Line Plots
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234821, "Etc/GMT+11", "Tag ID 234821", x_label = "Date/time (HST - 1) hr)", ylims = c(400, 0))

ggsave("234821_lineplot.png")
```
**************************************************************************
#BOXPLOTS
...
```{r}
brett_box_plot(Depths_234821, "Etc/GMT+11", "Tag ID 234821",
               x_label = "Hour (HST - 1 hr)", ylims = c(400, 0))
ggsave("234821_boxplot.png")
