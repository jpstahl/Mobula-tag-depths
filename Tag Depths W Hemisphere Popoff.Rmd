...
##To update code for a new tag, use find and replace all for 1) the tag Ptt number (for example replace 234818 with 234818) and 2) time zone (for example replace Etc/GMT+10 with Etc/GMT+11) in 1) for time conversion from UTC to popoff time zone (line 67), 2) line plot function, and 3) boxplot function. 
...
##Read in data.
```{r}
Depths_234818 <-read.csv(file="C:/R_git_stuff/Mobula-tag-depths/234818-Series.csv", header=TRUE, sep=",")

#head(Depths_234818)
#glimpse(Depths_234818)
#dim(Depths_234818) 
```
...
```{r}
Depths_234818$datetime = dmy_hms(paste(Depths_234818$Day,Depths_234818$Time))
```
...
Popoff_loc_234818 <- Popoff_loc %>% 
  filter(Ptt == 234818) %>%
  select(Popoff_lat, Popoff_long) %>%
  mutate(Ptt = as.integer(234818))

Popoff_loc_234818$Popoff_tz <- tz_lookup_coords(Popoff_loc_234818$Popoff_lat, Popoff_loc_234818$Popoff_long, method = "accurate")

Depths_234818 <- left_join(Depths_234818,Popoff_loc_234818, by = join_by (Ptt))
```
...
```{r}
Depths_234818$Popoff_datetime = Depths_234818$datetime %>%
...
z <- Depths_234818$Popoff_datetime #Create a new variable z with datetime info to update.
...
Depths_234818$Popoff_datetime <- z #update Popoff_datetime field with z variable.
```
...
```{r}
Depths_234818$Popoff_datetime[na_rows]
```
*****************************************************************************************************
#Line Plots
...
```{r}
brett_depth_plot_good_label_yaxis(Depths_234818, "Etc/GMT+10", "Tag ID 234818", x_label = "Date/time (HST))", ylims = c(400, 0))

ggsave("234818_lineplot.png")
```
...
brett_box_plot(Depths_234818, "Etc/GMT+10", "Tag ID 234818", x_label = "Hours (HST)", ylims = c(400, 0))

ggsave("234818_boxplot.png")
```
